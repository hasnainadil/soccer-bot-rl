...
class Game_env:
    ...
    def get_reward_another(self) -> float:
        # distance from ball to target_post
        distance_ball_target_post_one = math.sqrt((self.soccer_ball.body.position.x - self.screen.get_width()//2)**2 + (self.soccer_ball.body.position.y - (self.screen.get_height()//2 - const.FIELD_HEIGHT//2))**2)
        distance_ball_target_post_two = math.sqrt((self.soccer_ball.body.position.x - self.screen.get_width()//2)**2 + (self.soccer_ball.body.position.y - (self.screen.get_height()//2 + const.FIELD_HEIGHT//2))**2)
    ...
    def get_reward(self) -> float:
        ...
        reward_two += (0 - vertical_zone_id) * const.BASE_REWARD # opponent vertical zone id is negative of bot vertical zone id
        # print("Distance bot one:", distance_bot_one, "Distance bot two:", distance_bot_two)

        return reward_one, reward_two

    def play_step(self, action_one:tuple=None, action_two:tuple=None, render:bool=True):
        if not render:
            return
        self.step_count += 1
        ...
        done, ending_reward_one, ending_reward_two, player_one_score, player_two_score = self.render()
        ...
        # print("<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>")
        self.get_state()
        return reward_one, reward_two, self.player_one_score, self.player_two_score, self.bot_one_state, self.bot_two_state, done
    ...
    def get_state(self):
        ...
        self.bot_one_state = np.concatenate((self.ball_relative_pos_one, self.target_post_relative_pos_one, self.opponent_relative_pos_one))
        ...
        self.bot_two_state = np.concatenate((self.ball_relative_pos_two, self.target_post_relative_pos_two, self.opponent_relative_pos_two))
